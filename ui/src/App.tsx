...
function App() {
    ...
    const [modalInvoker, setModalInvoker] = React.useState<IModalInvoker>(
        defaultModalInvoker
    );
    ...
    function isTargetEmpty(event) {
        return Array.from(document.querySelectorAll(".rowEvent")).includes(
            event.target
        );
    }

    function expandCardDown(
        event: IEvent,
        pageYCoordinate: number,
        preview = true
    ) {
    ...
    function rowMouseUpHandler(dayOfWeek: Date, event) {
        ...
        if (isTargetEmpty(event)) {
            // Creating a new Card only happens if a card isn't already there.
    ...
    function carryCardOver(date: string, yCoord: number, preview = true) {
        ...
        const secondsToAdd =
            (dateFns.getTime(event.endTime) -
                dateFns.getTime(event.startTime)) /
            1000;
        ...
        <>
            <div className="App">
                <div className="main_content">
                    <Sidebar
                        createEventWithCurrentTime={createEventWithCurrentTime}
                    />
                    <div className="calendar_content">
                        ...
                        <div
                            ...
                            onDropCapture={containerDragDropHandler}>
                            {weekArray.map(
                                (dayOfWeek: Date, rowViewID: number) => (
                                    <Row
                                        dayOfWeek={dayOfWeek}
                                        key={rowViewID}
                                        idx={rowViewID}
                                        eventHandlers={{
                                            mouseUp: rowMouseUpHandler.bind(
                                                null,
                                                dayOfWeek
                                            ),
                                        }}
                                        rowHeight={boardState.viewportHeight}>
                                        {boardState.eventDayCollection[
                                            hashDate(dayOfWeek)
                                        ]?.eventCollection.map(
                                            (
                                                event: IEvent,
                                                ...
                                                <Card
                                                    ...
                                                    scale={
                                                        boardState.viewportHeight
                                                    }
                                                    grid={
                                                        boardState.heightIndex
                                                    }
                                                    eventHandlers={{
                                                        dragStart: cardDragStartHandler.bind(
                                                            null,
                                                            event
                                                        ),
                                                        click: cardClickHandler.bind(
                                                            null,
                                                            event
                                                        ),
                                                    }}
...
function SVGHour() {
    ...
    return (
        <div id="svg_root" className="svg_root">
            {svgRoot && (
                <svg
                    width={svgRoot.offsetWidth}
                    viewBox={`0 0 ${svgRoot.offsetWidth * 2} ${height * 2}`}>
...
function getNearestSlot(rawCoordinate: number, slots: number[]): number {
    ...
    let coord = -1;
    slots.forEach(function (rowCoord, idx) {
        // a binary search would work better. Whatever.
